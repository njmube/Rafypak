...
//			this.construirComentariosEImporteConLetra(comprobante, comentarios, datosExtra.getImporteichon());
	...
	private void construirEncabezado(Comprobante comprobante, Imagen imagen)
		...
		if (imagen != null) {
			...
			imgLogo = Image.getInstance("images/LogoRafypak.jpg");
	...
	private void construirComplementoComercioExterior(Comprobante cfdi, ComercioExterior complementoComExt,
		...
		// this.agregarChunkYNuevaLinea("N�mero de identificaci�n o registro
	...
	private void construirTablaDocumentosRelacionados(Comprobante comprobante, Pago pago) throws DocumentException{
		...
		agregarCeldaConFondo("UUID DR", fontUuid, tablaConceptos, true);
		
		agregarCeldaConFondo("Serie", fontUuid, tablaConceptos, true);

		agregarCeldaConFondo("Folio", fontUuid, tablaConceptos, true);

		agregarCeldaConFondo("Moneda DR", fontUuid, tablaConceptos, true);

		agregarCeldaConFondo("Método de pago DR", fontUuid, tablaConceptos, true);

		agregarCeldaConFondo("Num. Parcialidad", fontUuid, tablaConceptos, true);

		agregarCeldaConFondo("Importe saldo Anterior", fontUuid, tablaConceptos, true);

		agregarCeldaConFondo("Importe pagado", fontUuid, tablaConceptos, true);

		agregarCeldaConFondo("Importe Saldo Insoluto", fontUuid, tablaConceptos, true);
		...
		for(DoctoRelacionado doc:pago.getDoctoRelacionado()){
			...
			this.agregarCeldaSinBorde(doc.getSerie(), fontUuid, tablaConceptos, false);
			this.agregarCeldaSinBorde(doc.getFolio(), fontUuid, tablaConceptos, false);
			this.agregarCeldaSinBorde(doc.getMonedaDR().getValor(), fontUuid, tablaConceptos, false);
			this.agregarCeldaSinBorde(doc.getMetodoDePagoDR().getValor(), fontUuid, tablaConceptos, false);
			this.agregarCeldaSinBorde(doc.getNumParcialidad()+"", fontUuid, tablaConceptos, true);
			this.agregarCeldaSinBorde(doc.getImpSaldoAnt()+"", fontUuid, tablaConceptos, true);
			this.agregarCeldaSinBorde(doc.getImpPagado()+"", fontUuid, tablaConceptos, true);
			this.agregarCeldaSinBorde(doc.getImpSaldoInsoluto()+"", fontUuid, tablaConceptos, true);
		}
	...
	private void construirClienteProveedor(Pago pago, PdfPTable tablaPago, Comprobante comprobante){
		Phrase clienteproveedor = new Phrase();
		 ...
		 String aux= (pago.getRfcEmisorCtaOrd()!=null)?pago.getRfcEmisorCtaOrd():"";
		 agregarChunkYNuevaLinea("RFC cuenta ordenante: "+ aux, font3, clienteproveedor);
		 aux= (pago.getRfcEmisorCtaBen()!=null)?pago.getRfcEmisorCtaBen():"";
		 agregarChunkYNuevaLinea("RFC cuenta: "+aux, font3, clienteproveedor);
		...
		aux= (pago.getCtaOrdenante()!=null)?pago.getCtaOrdenante():"";
		 this.agregarChunkYNuevaLinea("Cuenta ordenante: "+ aux, font3, clienteproveedor2);
		 aux= (pago.getCtaBeneficiario()!=null)? pago.getCtaBeneficiario():"";
		 this.agregarChunkYNuevaLinea("Cuenta : "+ aux, font3, clienteproveedor2);
...
//		tablaLeyendaTotal.addCell(celdaTablaEtqTotal);
	...
	private void crearTablaDoctoRelacionado(Comprobante c) throws DocumentException{
		...
		for(CfdiRelacionado r: rs.getCfdiRelacionado()){
			...
			c.getCfdiRelacionados().getTipoRelacion().value();
			String rel=c.getCfdiRelacionados().getTipoRelacion().value().toString();
			switch(rel){
			case "01":
				agregarCelda("01-Nota de crédito de los documentos relacionados", fontConceptos, tablaIVA, true);
				break;
			case "02":agregarCelda("02-Nota de débito de los documentos relacionados", fontConceptos, tablaIVA, true);
			break;
			case "03":agregarCelda("03-Devolución de mercancía sobre facturas o traslados previos", fontConceptos, tablaIVA, true);
			break;
			case "04":agregarCelda("04-Sustitución de los CFDI previos", fontConceptos, tablaIVA, true);
			break;
			case "05":agregarCelda("05-Traslados de mercancias facturados previamente", fontConceptos, tablaIVA, true);
			break;
			case "06":agregarCelda("06-Factura generada por los traslados previos</option>", fontConceptos, tablaIVA, true);
			break;
			case "07":agregarCelda("07-CFDI por aplicación de anticipo", fontConceptos, tablaIVA, true);
			break;
			case "08":agregarCelda("08-Factura generada por pagos en parcialidades", fontConceptos, tablaIVA, true);
			break;
			case "09":agregarCelda("09-Factura generada por pagos diferidos", fontConceptos, tablaIVA, true);
			break;
			}
